    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_INFILE_SIZE_DATA_0: begin
                    rdata <= int_infile_size[31:0];
                end
                ADDR_INFILEDATA_DATA_0: begin
                    rdata <= int_infiledata[31:0];
                end
                ...
                ADDR_OUTDATA_DATA_1: begin
                    rdata <= int_outdata[63:32];
                end
            endcase
        end
    end
...
// int_infile_size[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_infile_size[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_INFILE_SIZE_DATA_0)
            int_infile_size[31:0] <= (WDATA[31:0] & wmask) | (int_infile_size[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_infiledata[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_INFILEDATA_DATA_0)
            int_infiledata[31:0] <= (WDATA[31:0] & wmask) | (int_infiledata[31:0] & ~wmask);
    end
end

// int_infiledata[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_infiledata[63:32] <= 0;
    else if (ACLK_EN) begin
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OUTDATA_DATA_0)
            int_outdata[31:0] <= (WDATA[31:0] & wmask) | (int_outdata[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_OUTDATA_DATA_1)
            int_outdata[63:32] <= (WDATA[31:0] & wmask) | (int_outdata[63:32] & ~wmask);
    end
end
